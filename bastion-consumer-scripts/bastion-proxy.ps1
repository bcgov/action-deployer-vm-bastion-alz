...
#   -ResourceGroup, -g   Resource group containing Bastion and VM
#   -BastionName,   -b   Name of the Azure Bastion host
#   -VmName,        -v   Name of the jumpbox VM
#   -SubscriptionId,-s   Azure subscription ID to use
#   -TenantId,      -t   Entra (Azure AD) tenant ID to sign in to
#   -Port,          -p   Starting SOCKS5 port
#
# USAGE
#   .\scripts\bastion-proxy.ps1 -ResourceGroup <resource-group> -BastionName <bastion-name> `
#       -VmName <vm-name> -SubscriptionId <subscription-id> -TenantId <tenant-id> -Port <port>
#
# EXAMPLE
#   .\scripts\bastion-proxy.ps1 `
...
if (-not (Test-AzLogin)) {
    ...
    if ($currentTenantId -and $currentTenantId -ne $TenantId) {
    ...
    Write-Warn 'Already logged in. Azure CLI sessions expire 12h after Azure CLI login.'
    Write-Warn 'Re-run this script if you encounter authentication errors.'
}

$currentTenantId = az account show --query tenantId --output tsv
if ($currentTenantId -ne $TenantId) {
    Write-Err "Azure CLI is not using the requested tenant $TenantId."
    exit 1
}
...
Write-Ok "Authenticated to Azure CLI (tenant $currentTenantId)"
