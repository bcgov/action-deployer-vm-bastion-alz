...
#     -s <subscription-id> -t <tenant-id> -p <port>
#
...
#   -s, --subscription      Azure subscription ID to use               [required]
#   -t, --tenant            Entra (Azure AD) tenant ID to sign in to   [required]
#   -p, --port              Starting SOCKS5 port                       [required]
#   -h, --help              Show this help and exit
#
# EXAMPLES
#   # Entra ID (AAD) auth — every parameter supplied explicitly:
...
[[ -z "$SUBSCRIPTION_ID" ]] && { err "--subscription (-s) is required";   exit 1; }
[[ -z "$TENANT_ID" ]]       && { err "--tenant (-t) is required";         exit 1; }
[[ -z "$START_PORT" ]]      && { err "--port (-p) is required";           exit 1; }
...
if ! az account show &>/dev/null 2>&1; then
  ...
  if [[ -n "$CURRENT_TENANT_ID" && "$CURRENT_TENANT_ID" != "$TENANT_ID" ]]; then
    echo ""
...
CURRENT_TENANT_ID=$(az account show --query tenantId --output tsv | trim_cr)
[[ "$CURRENT_TENANT_ID" == "$TENANT_ID" ]] || { err "Azure CLI is not using the requested tenant ${TENANT_ID}."; exit 1; }
