...
variable "availability_zone" {
  description = "Availability zone to pin the jumpbox VM to (\"1\", \"2\", or \"3\"). null (the default) keeps the VM non-zonal, matching the original deployment. Changing this on an existing VM forces replacement."
  type        = string
  default     = null
...
variable "vm_admin_login_principal_ids" {
  description = "Microsoft Entra object IDs (users or groups) granted the Virtual Machine Administrator Login role on the jumpbox. At least one is required for anyone to Entra-SSH in; an empty list grants no access. Values must be object IDs (GUIDs)."
  type        = list(string)
  default     = []
  nullable    = false
}
