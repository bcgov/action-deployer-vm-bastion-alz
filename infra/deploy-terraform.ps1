...
#
# Auto-install: Terraform, the Azure CLI, and (for standalone `deploy` or
# `destroy` runs) Git are installed automatically if missing. Each tool is attempted via WinGet,
...
param(
    ...
    # Branch or tag a standalone `deploy` or `destroy` self-clones when this
    # script has no infra/ checkout next to it (downloaded directly via
...
# `deploy` and `destroy` must work when this script is downloaded standalone
# (e.g. via raw.githubusercontent.com, no repo clone) -- self-clone the repo
# with Git so the rest of infra/ (main.tf, modules/, etc.) is available to run
# Terraform against. Every other command keeps assuming $PSScriptRoot is a real
# checkout, unchanged.
...
# Safe to delete; git-ignored; regenerated automatically by local `deploy` or `destroy`.
terraform {
  backend "local" {}
}
'@
...
function Confirm-BackendConsistency {
    param(
        [Parameter(Mandatory)][ValidateSet('azurerm', 'local')][string]$BackendMode,
        [switch]$AllowLocalStateForDestroy
    )
    ...
    if (-not $overridePresent -and $BackendMode -eq 'local') {
        if ($AllowLocalStateForDestroy) {
            Write-DeployLog 'No BACKEND_* environment variables are set; destroy will use LOCAL Terraform state.'
            Set-LocalBackendOverride -InfraPath $InfraDir -BackendMode 'local'
        ...
        default {
            ...
            if ($Command -ne 'deploy') {
                Confirm-BackendConsistency -BackendMode $script:BackendMode `
                    -AllowLocalStateForDestroy:($Command -eq 'destroy')
            }
        }
    }
